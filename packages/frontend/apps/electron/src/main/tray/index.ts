...
class TrayState implements Disposable {
  ...
  getSecondaryMenuProvider(): TrayMenuProvider {
    return {
      ...
      getConfig: () => [
        {
          label: 'Open ClickDz Work',
          click: () => {
            logger.info('User action: Open ClickDz Work');
            getMainWindow()
              .then(w => {
                w.show();
              })
              .catch(err => {
                logger.error('Failed to open ClickDz Work:', err);
              });
          },
        },
        {
        ...
        'separator',
        {
          label: 'Quit ClickDz Work Completely...',
          click: () => {
            logger.info('User action: Quit ClickDz Work Completely');
            app.quit();
          },
        },
      ],
