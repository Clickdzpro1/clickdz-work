// ClickDz Work — Feature / Section / Module ID registry (FRONTEND MIRROR).
//
// SYNC: mirror of backend clickdz-features.ts — keep in lockstep.
//
// This file is a BYTE-IDENTICAL copy (below this header) of the canonical
// registry at packages/backend/server/src/plugins/copilot/clickdz-features.ts,
// which is the SINGLE SOURCE OF TRUTH for every feature/section/module id
// (binding decision C2). It is duplicated here — not imported — because in this
// yarn-workspace layout packages/frontend/core has NO dependency on
// packages/backend/server and no frontend file imports from plugins/copilot, so
// a cross-package import would not resolve at build time. The module is
// dependency-light BY CONTRACT (types + data + pure helpers, no React / Nest /
// Node imports), which is exactly what makes an identical mirror safe.
//
// WHEN YOU EDIT ONE, EDIT BOTH. The two files' code bodies must stay identical;
// only this header differs. (Recommend an orchestrator/CI lint that diffs the
// two bodies and fails on drift — this is how the pre-existing FE 3-id vs BE
// 4-id SECTION drift is prevented from ever recurring.)
