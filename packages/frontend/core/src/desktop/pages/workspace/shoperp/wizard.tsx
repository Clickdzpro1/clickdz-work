...
export const ShopWizard = ({ onDone, onCancel, hasExistingApps }: {
  ...
  if (phase.kind === 'done') { return <DoneCard result={phase.result} settings={settings} onFinish={onDone} />; }
...
const Card = ({ children }: { children: ReactNode }) => (
  <div style={{ maxWidth: 560, margin: '0 auto', padding: 24, borderRadius: 16, background: C.panel, border: '1px solid ' + C.border, display: 'flex', flexDirection: 'column', boxShadow: '0 1px 4px rgba(0,0,0,0.12)' }}>{children}</div>
);
...
const StepShell = ({ emoji, title, subtitle, children }: { emoji: string; title: string; subtitle: string; children?: ReactNode }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <div aria-hidden style={{ width: 48, height: 48, borderRadius: 14, display: 'grid', placeItems: 'center', fontSize: 24, background: 'linear-gradient(135deg, var(--affine-primary-color, #1e96eb), color-mix(in srgb, var(--affine-primary-color, #1e96eb) 70%, #000))', border: '1px solid ' + C.border }}>
        {emoji}
      </div>
  ...
  return (
    <Card>
      <StepShell emoji="⚠️" title="Limite de publication atteinte" subtitle={'Votre workspace peut garder ' + info.limit + ' application' + (info.limit === 1 ? '' : 's') + ' publiée' + (info.limit === 1 ? '' : 's') + '. Choisissez une à remplacer.'}>
        {info.existing.length === 0 ? <Banner tone="warn">Aucune application à remplacer.</Banner> : <div style={{ display: 'flex', flexDirection: 'column', gap: 2, borderRadius: 12, overflow: 'hidden', border: '1px solid ' + C.border }}>{info.existing.map(app => { const on = chosen === app.slug; return <button key={app.slug} type="button" onClick={() => setChosen(app.slug)} style={{ appearance: 'none', textAlign: 'left', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10, padding: '11px 14px', background: on ? C.accentSoft : C.bg, border: 'none', borderInlineStart: '3px solid ' + (on ? C.accent : 'transparent'), color: C.text }}><span style={{ width: 14, height: 14, borderRadius: '50%', flexShrink: 0, border: '2px solid ' + (on ? C.accent : C.border), background: on ? C.accent : 'transparent' }} /><span style={{ flex: 1, fontSize: 13, fontWeight: 600, fontFamily: 'var(--affine-font-code-family, monospace)' }}>{app.slug}</span></button>; })}</div>}
  ...
  return (
    <Card>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center', textAlign: 'center' }}>
        <div aria-hidden style={{ width: 56, height: 56, borderRadius: 16, display: 'grid', placeItems: 'center', fontSize: 28, background: 'linear-gradient(135deg, var(--affine-primary-color, #1e96eb), color-mix(in srgb, var(--affine-primary-color, #1e96eb) 70%, #000))', border: '1px solid ' + C.border }}>
          🎉
        </div>
...
const ResultTile = ({ emoji, title, subtitle, accent, children }: { emoji: string; title: string; subtitle: string; accent: string; children?: ReactNode }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 14, borderRadius: 14, background: C.bg, border: '1px solid ' + C.border, boxShadow: '0 1px 3px rgba(0,0,0,0.10)' }}>
    <div style={{ width: 40, height: 40, borderRadius: 12, flexShrink: 0, display: 'grid', placeItems: 'center', fontSize: 20, background: 'color-mix(in srgb, ' + accent + ' 22%, transparent)' }} aria-hidden>{emoji}</div>
