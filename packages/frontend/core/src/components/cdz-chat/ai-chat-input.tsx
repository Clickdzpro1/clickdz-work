...
/**
 * CDZ Sidechat — AI chat input (PromptInput).
 *
 * Adapted from the 21st.dev ai-chat-input component for the ClickDz Work
 * (AFFiNE fork) codebase. Changes from the upstream component:
 *
 *  1. `import { cn } from "@/lib/utils"` -> `import { cn } from "./cn"` (AFFiNE
 *     has no `@/` alias or `@/lib/utils`; we ship a clsx-based `cn`).
 *  2. `models?: string[]` -> `models?: CdzModelOption[]` from `./cdz-models`.
 *     The selector now cycles real CDZ supermodels (cdz-ultra, cdz-council,
 *     cdz-sage, ...) and passes the CDZ model *id* (not a display string) to
 *     onSubmit, so the parent can call the CDZ AI API directly.
 *  3. Voice (mic) button is conditionally rendered via
 *     `useCdzBrowserCapabilities` — hidden on iOS Safari and Firefox where
 *     SpeechRecognition is unreliable/unavailable, per the browser audit.
 *  4. On mount we inject the scoped Tailwind layer (`./cdz-chat.css`) and the
 *     namespaced `--cdz-*` design tokens (`./cdz-tokens`), mirroring the
 *     existing clickdz/workspace-boot.tsx <style> injection pattern.
 *  5. The component renders inside a `.cdz-chat-scope` wrapper so the scoped
 *     Tailwind utilities + base resets cannot leak into the rest of the app.
 *
 * Everything else (MorphingText, effort selector, attachment gallery FLIP
 * animation, voice visualizer, simulated fallbacks) is preserved verbatim.
 */
...
import { useCdzBrowserCapabilities } from './use-cdz-browser-capabilities';

// ----------------------------------------------------------------------
// Transition physics (unchanged from upstream)
// ----------------------------------------------------------------------
...
/**
 * Model icon. Upstream pulled remote SVGs from Cloudinary keyed by display
 * label. For CDZ we keep a small inline glyph map keyed by CDZ model id so we
 * don't depend on an external CDN, and fall back to a neutral dot.
 */
function ModelIcon({
  model,
  className,
}: {
  model: CdzModelOption;
  className?: string;
}) {
  // Inline SVG glyphs per vendor — crisp at any size, no network.
  const Glyph = (): React.ReactNode => {
    switch (model.vendor) {
      case 'anthropic':
        return (
          <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <path d="M14.16 4h-2.32l4.8 16h2.32L14.16 4zM9.68 4H7.36l4.8 16h2.32L9.68 4z" />
            <path d="M17.6 20h2.4L13.6 4h-2.4l6.4 16zM9.6 4 3.2 20h2.4l6.4-16H9.6z" />
          </svg>
        );
      case 'openai':
        return (
          <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <path d="M22.28 9.82a5.94 5.94 0 0 0-.51-4.91 6 6 0 0 0-6.5-2.94A6 6 0 0 0 4.93 4.18a5.94 5.94 0 0 0-3.97 2.88 6 6 0 0 0 .74 7.03 5.94 5.94 0 0 0 .51 4.91 6 6 0 0 0 6.5 2.94 6 6 0 0 0 4.51 2.02 6 6 0 0 0 5.77-4.18 5.94 5.94 0 0 0 3.97-2.88 6 6 0 0 0-.74-7.03z" />
          </svg>
        );
      case 'google':
        return (
          <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <path d="M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20zm3.6 8.6a3.6 3.6 0 1 1-7.2 0 3.6 3.6 0 0 1 7.2 0z" opacity="0.9" />
          </svg>
        );
      case 'cdz':
      default:
        // Infinity-ish compass for the CDZ supermodels.
        return (
...
// Main Component
// ----------------------------------------------------------------------
export interface CdzPromptInputProps {
  onSubmit?: (
    value: string,
  ...
  placeholder?: string;
  className?: string;
  /** CDZ model list. Defaults to the 7 CDZ supermodels + Council. */
  models?: CdzModelOption[];
  /** Effort levels. Defaults to ['Low','Medium','Max Effort']. */
  efforts?: string[];
  defaultValue?: string;
  value?: string;
...
export const CdzPromptInput = React.forwardRef<HTMLDivElement, CdzPromptInputProps>(
  (
    {
      onSubmit,
      placeholder = 'Ask CDZ anything',
      className,
      models = CDZ_MODELS,
      efforts = [...CDZ_EFFORTS],
      defaultValue = '',
    ...
    // Inject the scoped Tailwind layer + --cdz-* tokens once on mount.
    useEffect(() => {
      ensureCdzChatStyles();
    }, []);

    const capabilities = useCdzBrowserCapabilities();

    const [expanded, setExpanded] = useState(false);
    const [isSmoothResize, setIsSmoothResize] = useState(false);
    const [localValue, setLocalValue] = useState(defaultValue);
    const [selectedModel, setSelectedModel] = useState<CdzModelOption>(
      models.find(m => m.id === CDZ_DEFAULT_MODEL_ID) ?? models[0]
    );
    ...
    const updateFades = () => {
      ...
      if (bottomFadeRef.current) {
        ...
        bottomFadeRef.current.style.opacity = Math
          .min(Math.max(bottomScroll - 16, 0) / 10, 1)
          .toString();
      }
    };
    ...
    const startRecording = useCallback(async () => {
      ...
      try {
        ...
        // Mic denied / unavailable — fall back to simulated mode (below).
      }
      setIsRecording(true);
      ...
      if (stream) {
        ...
        // Fallback simulated visualizer (e.g. mic permission denied).
    ...
    const showMic = !hasValue && !isRecording && capabilities.micButtonVisible;
    const onActionButtonClick = (e: React.MouseEvent) => {
      ...
      if (isRecording) {
        ...
        handleSubmit();
      } else if (capabilities.micButtonVisible) {
        startRecording();
      }
    };
    ...
    return (
      <>
        ...
        <div
          ...
          onBlur={handleBlur}
          className={cn('cdz-chat-scope relative flex flex-col w-full', className)}
          style={{
...
CdzPromptInput.displayName = 'CdzPromptInput';
export default CdzPromptInput;
