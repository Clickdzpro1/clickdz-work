...
export const APP_BUILDER_GUIDELINES = [
  ...
  '  const WRITE = { "Content-Type": "application/json", ...AUTH };',
  '  save:   await (await fetch(`${DATA}/items`, { method: "POST",',
  '            headers: WRITE, body: JSON.stringify(record) })).json()',
  '  load:   await (await fetch(`${DATA}/items`, { headers: AUTH })).json()',
  '            // newest first',
  '  remove: await fetch(`${DATA}/items/${id}`, { method: "DELETE",',
  '            headers: AUTH })',
  '- IMPORTANT: send the Authorization: Bearer ${TOKEN} header on EVERY request,',
  '  reads included. Writes and deletes are refused with 401 without it, and',
  '  reads of collections that hold customer data (orders, customers, clients,',
  '  caisse, expenses, creances, invoices…) are refused too. Sending it on a',
  '  read that does not require it is harmless. Always keep the injected TOKEN',
  '  placeholder.',
...
export const SHOP_CONTRACT_RULES = [
  ...
  '2. THE BEARER AUTH — ON EVERY REQUEST, READS INCLUDED. All writes (POST),',
  '     deletes (DELETE) AND reads (GET) MUST keep the',
  '     Authorization: Bearer <DATA_TOKEN> header. In the storefront that is',
  "     h['Authorization'] = 'Bearer ' + DATA_TOKEN; inside the api client, and",
  '     api.list() deliberately calls headers(true) so reads carry it too — do',
  '     NOT "optimise" that away. Dropping the header makes writes 401 and',
  '   silently loses every order, and makes reads of order/customer collections',
  '   401 once the platform read gate is on. Keep the api data-client intact.',
