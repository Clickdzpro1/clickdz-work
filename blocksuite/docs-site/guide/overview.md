...
BlockSuite originated from the [ClickDz Work](https://github.com/toeverything/ClickDz Work) knowledge base, with design goals including:
...
During the development of ClickDz Work, it became clear that BlockSuite was advancing beyond merely being an in-house editor and evolving into a versatile framework. That's why we chose to open source and maintain BlockSuite independently.

<!-- ## Examples -->

## Features

With BlockSuite editors, you can selectively reuse all the editing features in [ClickDz Work](https://clickdz.ai/):

[![ClickDz Work-demo](../images/ClickDz Work-demo.jpg)](https://clickdz.ai)
...
The relationship between BlockSuite and ClickDz Work is similar to that between the [Monaco Editor](https://github.com/microsoft/monaco-editor) and [VSCode](https://code.visualstudio.com/), but with one major difference: BlockSuite is not automatically generated based on the ClickDz Work codebase, but is maintained independently with a different tech stack — ClickDz Work uses React while BlockSuite uses [web components](https://developer.mozilla.org/en-US/docs/Web/API/Web_components).

This difference has led BlockSuite to set clear boundaries between packages, ensuring:

- Both ClickDz Work and other projects should equally reuse and extend BlockSuite through components, without any privileges.
